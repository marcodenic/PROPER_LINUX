...
private slots:
    ...
    void select(int) { auto o=current(); if(o.isEmpty()) return; QStringList arch; for(auto v:o["architectures"].toArray()) arch<<v.toString(); const auto installed=isInstalled(o); install->setText(installed ? "Installed" : "Install"); install->setEnabled(!installed); launch->setEnabled(installed); advanced->setChecked(false); detail->setHtml(QString("<h2>%1</h2><p>%2</p><p><b>Category:</b> %3</p><p><b>State:</b> %8</p>").arg(o["name"].toString(),o["description"].toString(),o["category"].toString(),o["provider"].toString(),o["status"].toString(),o["license"].toString(),arch.join(", "),installed ? "Installed" : "Available")); sourceDetail->setHtml(QString("<p><b>Provider:</b> %1<br><b>Status:</b> %2<br><b>Licence:</b> %3<br><b>Architecture:</b> %4</p>").arg(o["provider"].toString(),o["status"].toString(),o["license"].toString(),arch.join(", "))); }
