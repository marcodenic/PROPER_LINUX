...
private slots:
    void refresh() { list->clear(); auto q=search->text().toLower(); auto category=categories->currentText(); for (const auto &v: entries) { auto o=v.toObject(); auto hay=(o["name"].toString()+" "+o["category"].toString()+" "+o["description"].toString()).toLower(); if ((category=="All categories"||o["category"].toString()==category) && (q.isEmpty()||hay.contains(q))) { auto *i=new QListWidgetItem(o["name"].toString()+"  ·  "+o["category"].toString()); i->setData(Qt::UserRole, o["id"].toString()); list->addItem(i); } } if (list->count()) list->setCurrentRow(0); else { advanced->setChecked(false); sourceDetail->clear(); detail->setHtml("<h2>No applications found</h2><p>Try another search.</p>"); install->setEnabled(false); launch->setEnabled(false); recover->setEnabled(false); } }
    ...
    void select(int) { auto o=current(); if(o.isEmpty()) return; QStringList arch; for(auto v:o["architectures"].toArray()) arch<<v.toString(); const auto installed=isInstalled(o); install->setText(installed ? "Installed" : "Install"); install->setEnabled(!installed); launch->setEnabled(installed); recover->setEnabled(installed); advanced->setChecked(false); detail->setHtml(QString("<h2>%1</h2><p>%2</p><p><b>Category:</b> %3</p><p><b>State:</b> %8</p>").arg(o["name"].toString(),o["description"].toString(),o["category"].toString(),o["provider"].toString(),o["status"].toString(),o["license"].toString(),arch.join(", "),installed ? "Installed" : "Available")); sourceDetail->setHtml(QString("<p><b>Provider:</b> %1<br><b>Status:</b> %2<br><b>Licence:</b> %3<br><b>Architecture:</b> %4</p>").arg(o["provider"].toString(),o["status"].toString(),o["license"].toString(),arch.join(", "))); }
...
private: QJsonArray entries; QLineEdit *search{}; QComboBox *categories{}; QListWidget *list{}; QTextBrowser *detail{},*sourceDetail{}; QToolButton *advanced{}; QPushButton *install{},*launch{},*recover{}; QProcess *proc{};
};
#include "main.moc"
