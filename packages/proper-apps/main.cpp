...
private slots:
    ...
    bool isInstalled(const QJsonObject &o) const { const auto provider=o["provider"].toString(); const auto id=o["provider_id"].toString(); if (provider=="flatpak") return QProcess::execute("flatpak", {"info", id})==0; if (provider=="fedora") return QProcess::execute("rpm", {"-q", id})==0; const auto launch=o["launch_args"].toArray(); return !launch.isEmpty() && !QStandardPaths::findExecutable(launch.first().toString()).isEmpty(); }
    ...
    void run(QStringList args, bool privileged=false) { if(args.isEmpty()) return; install->setEnabled(false); detail->append("<p><b>Working…</b></p>"); proc=new QProcess(this); connect(proc, qOverload<int,QProcess::ExitStatus>(&QProcess::finished), this, [this](int code,QProcess::ExitStatus){ install->setEnabled(true); if(code==0) detail->append("<p><b>Installed.</b> You can launch it now.</p>"); else detail->append("<p><b>Installation failed.</b> The provider made no confirmed change. Try again or use recovery.</p>"); }); QString program=args.takeFirst(); if(privileged) { QStringList p; p<<program<<args; proc->start("pkexec",p); } else proc->start(program,args); }
    void runSteps(const QJsonArray &steps, int index=0) { if(index>=steps.size()) { install->setEnabled(true); detail->append("<p><b>Installed.</b> You can launch it now.</p>"); return; } auto s=steps[index].toObject(); QString program=s["program"].toString(); QStringList args; for(auto v:s["args"].toArray()) args<<v.toString(); if(program.isEmpty()) { detail->append("<p><b>Installation failed.</b> Invalid provider step.</p>"); install->setEnabled(true); return; } detail->append(QString("<p>Step %1 of %2…</p>").arg(index+1).arg(steps.size())); proc=new QProcess(this); connect(proc, qOverload<int,QProcess::ExitStatus>(&QProcess::finished), this, [this,steps,index](int code,QProcess::ExitStatus){ if(code!=0) { install->setEnabled(true); detail->append("<p><b>Installation failed.</b> The provider made no confirmed change. Try again or use recovery.</p>"); return; } runSteps(steps,index+1); }); bool privileged=s["privileged"].toBool(); if(privileged) { QStringList p; p<<program<<args; proc->start("pkexec",p); } else proc->start(program,args); }
    ...
    void doLaunch(){ auto o=current(); auto a=o["launch_args"].toArray(); QStringList args; for(auto v:a) args<<v.toString(); if(!args.isEmpty()) run(args); }
    void doRecover(){ QMessageBox::information(this,"Recovery","The provider's normal uninstall/recovery action is available through Fedora Software and Flatpak. No credentials are stored by Proper Apps."); }
