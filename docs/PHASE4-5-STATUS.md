...
- Authoritative reinstall-persistence evidence uses the development-only auxiliary RPM media path supported by `scripts/run-vm`: in fresh guest `/home/code/.codex/worktrees/7b43/proper-linux-build-icons/vm/persistence-1787765000/`, user config was changed to `font-size = 13`, the exact `proper-terminal-1.2.3-5.fc44.x86_64.rpm` was reinstalled with `rpm -Uvh --replacepkgs` (`rpm-install2.png`, SHA-256 `bea6ea880a25a67928620e9f01d9fd06c3aabd0e2d3836eabec520f6de9e925d`), and the setting remained afterward (`persistence-final.png`, SHA-256 `211a895b0cfee5beb5e8b1e1ffea457c191714ffc8e75decefd6aadec8ed4b72`).
...
| Fresh-user defaults and upgrade persistence | Fresh defaults are present; user Ghostty setting survived exact RPM reinstall in auxiliary-media guest |
